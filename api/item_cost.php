...
function pickItem(name, supplier, imageUrl, price) {
    ...
    if (el) {
        ...
        el.querySelector('[data-field="price"]').value = price || '';  // ← auto-fill price
        calcRow(pickerTarget);  // ← i-recalculate agad ang total
...
function renderPicker(items) {
    ...
    Object.keys(groups).sort().forEach(s => {
        ...
        groups[s].forEach(it => {
            html += `<div class="picker-item" onclick="pickItem('${escJs(it.item_name)}','${escJs(it.supplier_name)}','${escJs(it.image_url || '')}','${escJs(it.price || '')}')">
